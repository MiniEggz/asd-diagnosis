...
from sklearn.utils import check_array, column_or_1d, compute_sample_weight
    ...
    def _validate_data(
        ...
        **check_params : kwargs
        ...
        if no_val_X and no_val_y:
            ...
            X = check_array(
                X, ensure_2d=False, allow_nd=True, input_name="X", **check_params
            )
            out = X
            ...
            if validate_separately:
                ...
                X = check_array(
                    X, ensure_2d=False, allow_nd=True, input_name="X", **check_X_params
                )
...
class _RemursClassifierMixin(LinearClassifierMixin):
    def _prepare_data(self, X, y, sample_weight, solver):
        ...
        sample_weight = _check_sample_weight(
            sample_weight, X.reshape((X.shape[0], -1)), dtype=X.dtype
        )
    ...
    def decision_function(self, X):
        ...
        scores = (
            safe_sparse_dot(
                X.reshape((X.shape[0], coef.shape[0])), coef, dense_output=True
            )
            + self.intercept_
        )
        try:
